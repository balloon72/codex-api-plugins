...
def run_gh_command(args: Sequence[str], cwd: Path) -> GhResult:
    if GH_EXE is None:
        return GhResult(1, "", GH_INSTALL_HINT)
    last_result = GhResult(1, "", "")
    for attempt in range(1, GH_RETRY_ATTEMPTS + 1):
        ...
        result = GhResult(process.returncode, process.stdout, process.stderr)
        if result.returncode == 0:
            return result
        message = (result.stderr or result.stdout or "").strip()
        last_result = result
        if attempt < GH_RETRY_ATTEMPTS and is_retryable_gh_error(message):
...
def run_gh_command_raw(args: Sequence[str], cwd: Path) -> tuple[int, bytes, str]:
    ...
    for attempt in range(1, GH_RETRY_ATTEMPTS + 1):
        ...
        current = (process.returncode, process.stdout, stderr)
        if process.returncode == 0:
            return current
        message = (stderr or process.stdout.decode(errors="replace")).strip()
        last = current
        if attempt < GH_RETRY_ATTEMPTS and is_retryable_gh_error(message):
            time.sleep(GH_RETRY_DELAY_SECONDS)
            continue
...
def main() -> int:
    ...
    if not ensure_gh_available(repo_path):
        return 1
...
def ensure_gh_available(repo_root: Path) -> bool:
    if GH_EXE is None:
        print(f"Error: {GH_INSTALL_HINT}", file=sys.stderr)
        return False
    version_result = run_gh_command(["--version"], cwd=repo_root)
    if version_result.returncode != 0:
        message = (version_result.stderr or version_result.stdout or "").strip()
        print(message or f"Error: {GH_INSTALL_HINT}", file=sys.stderr)
        return False
    result = run_gh_command(["auth", "status"], cwd=repo_root)
    if result.returncode == 0:
        return True
    message = (result.stderr or result.stdout or "").strip()
    print(message or f"Error: {GH_LOGIN_HINT}", file=sys.stderr)
    return False
